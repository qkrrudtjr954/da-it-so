...
public class AbilityDao implements AbilityDaoImpl {
	...
	public boolean addAbility(AbilityBbs abilityDto, Person personDto) {
		...
		if (DBConnector.getClass().getName().equals("db.MySqlConnection")) {
			...
			sql = "INSERT INTO ABILITY_BBS(SEQ, CATEGORY_ID, TITLE, IMGURL1, IMGURL2, IMGURL3, IMGURL4, ABILITY, CONTENT, STATE, CREATE_AT, USER_ID)"
					+ " VALUES(IBBS_SEQ.NEXTVAL, '" + abilityDto.getCategory_id() + "','" + abilityDto.getTitle()
					+ "','" + abilityDto.getImgurl1() + "','" + abilityDto.getImgurl2() + "','"
					+ abilityDto.getImgurl3() + "','" + abilityDto.getImgurl4() + "','" + abilityDto.getAbility()
					+ "','" + abilityDto.getContent() + "', 1, SYSDATE, '" + personDto.getId() + "')";
	...
	public List<AbilityBbs> search(AbilityBbs Adto) {
		...
		String sql = " SELECT * FROM ABILITY_BBS " + " WHERE TITLE = '" + Adto.getTitle() + "'";
